...
float calc_it (float ina, float inb, float f_weight, float s_weight, float bias) {
...
printf("Inputs to neurons are: %f, %f\n", input[0], input[1]);
  ...
  float out[2];
  float o_out[2];
  for(int ll; ll < 10000; ll++) {  // learn loop
  ...
  //float total_output = calc_it(out[1], out[2], weight[4], weight[5], bias_b);
  //printf("%f", total_output);
  //printf("OUT1: %f OUT2: %f OUTo1: %f OUTo2: %f\n", out[1], out[2], o_out[1], o_out[2]);
  ...
  //printf("Target minus output %f %f\n", tar_less_out[1], der[1]);
  //printf("Target minus output %f %f\n", tar_less_out[2], der[2]); 
  float diff[8];
  float bp[2];
  // delta total
  float total[2];
  ...
  nweight[4] = weight[4] - lr * total[1];
  nweight[5] = weight[5] - lr * total[1];
  nweight[6] = weight[6] - lr * total[2];
  nweight[7] = weight[7] - lr * total[2];
  // start the back prop
  ...
  weight[0] = weight[0] - lr * backout;
  weight[1] = weight[1] - lr * backout;
  weight[2] = weight[2] - lr * backout;
  weight[3] = weight[3] - lr * backout;
  weight[4] = nweight[4];
  weight[5] = nweight[5];
  weight[6] = nweight[6];
  weight[7] = nweight[7];
